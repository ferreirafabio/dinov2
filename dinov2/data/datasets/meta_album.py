...
class MetaAlbum(Dataset):
    ...
    def __init__(self,
        ...
        if self.split == "train":
            self.split_idxs = self.idxs[split_test_idx:]
        elif self.split == "val":
            self.split_idxs = self.idxs[:split_val_idx]
        elif self.split == "test":
            self.split_idxs = self.idxs[split_val_idx:split_test_idx]
        else:
            raise ValueError("Not implemented split.")

        self.items = [item for i, item in enumerate(self.items) if i in self.split_idxs.tolist()]
    ...
    def get_targets(self) -> Optional[np.ndarray]:
        return None if self.split == "test" else self.split_idxs
